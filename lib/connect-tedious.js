...
module.exports = function(connect){
  ...
  */
  function dbCleanup(store, fn) {
    store.pool.requestConnection(function(err, db) {
        if (err)
          return fn(err);
        initConnection(db);
  ...
  function TediousStore(options, connectionString) {
    ...
    if (builder.dataSource) {
        ...
        options.config.options.instanceName=ds[1] || options.config.options.instanceName; 
    }
    ...
    options.config.encrypt=builder.encrypt || options.config.encrypt;
