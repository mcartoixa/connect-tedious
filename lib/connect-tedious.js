...
module.exports = function(connect){
  ...
  TediousStore.prototype.get = function(sid, fn) {
    ...
    var r = new tedious.Request(
      'SELECT s.' + this.sessColumnName + ' FROM ' + this.tableName + ' WHERE ' + this.sidColumnName + '=@sid AND ' + this.expiredColumnName + '>=@now',
    ...
    r.addParameter('now', TYPES.DateTime, now);
  ...
  TediousStore.prototype.set = function(sid, sess, fn){
    ...
    var r = new tedious.Request(
      ...
      '  WHEN MATCHED THEN UPDATE SET s.' + this.sessColumnName + '=@session, s.' + this.expiredColumnName + '=@expired' +
      '  WHEN NOT MATCHED THEN INSERT (' + this.sidColumnName + ', ' + this.sessColumnName + ', ' + this.expiredColumnName + ') VALUES (@sid, @sess, @expired)',
      function(err) {
        if (err)
          return fn(err);
      }
    ...
    r.addParameter('expired', TYPES.DateTime, expired);
    this.connection.execSql(r);
  };
